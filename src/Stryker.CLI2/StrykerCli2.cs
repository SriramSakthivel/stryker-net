...
public class StrykerCli2
{
    ...
    private readonly Lazy<IProjectOrchestrator> _lazyProjectOrchestrator;

    public int ExitCode { get; private set; } = ExitCodes.Success;

    public StrykerCli2(IStrykerRunner stryker = null,
        Lazy<IProjectOrchestrator> lazyProjectOrchestrator = null,
        IConfigBuilder configReader = null,
        ILoggingInitializer loggingInitializer = null,
        IStrykerNugetFeedClient nugetClient = null,
        IAnsiConsole console = null)
    {
        _lazyProjectOrchestrator = lazyProjectOrchestrator;
    ...
    public int Run(string[] args)
    {
        var app = new CommandLineApplication
        {
            ...
            HelpTextGenerator = CreateHelpTextGenerator() 
        };
        app.HelpOption();
    ...
    private static IHelpTextGenerator CreateHelpTextGenerator()
    {
        IHelpTextGenerator helpTextGenerator = (IHelpTextGenerator) typeof(StrykerCli).Assembly.GetType("Stryker.CLI.GroupedHelpTextGenerator")?.GetConstructor(Type.EmptyTypes)?.Invoke(null);
        return helpTextGenerator;
    }

    private void RunStryker(IStrykerInputs inputs)
    {
        Stryker.Core.Logging.ApplicationLogging.LoggerFactory = Stryker.CLI.Logging.ApplicationLogging.LoggerFactory;
        var projectOrchestrator = _lazyProjectOrchestrator?.Value;
        var result = _stryker.RunMutationTest(inputs, ApplicationLogging.LoggerFactory, projectOrchestrator);
